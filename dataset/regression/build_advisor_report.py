"""Build an advisor-ready single-page report from the v2 per-pair regression.
...
Writes:
  outputs/advisor_report.md
  outputs/advisor_coefficient_table.csv
  outputs/advisor_variance_decomposition.csv
  outputs/advisor_vif_table.csv
"""
...
def render_report(df, model, coef_df, var_df, vif_df, multi, resid):
    ...
    lines = []
    lines.append("# IRR Regression — Advisor Brief")
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append(f"```")
    lines.append(f"pct_agreement ~ C(coder_a) + C(coder_b) + C(question)")
    lines.append(f"               + number_coded_prior_a + number_coded_prior_b")
    lines.append(f"               + is_legacy_volume")
    lines.append("```")
    lines.append("")
    lines.append("Volume fixed effects were intentionally OMITTED so that `number_coded_prior` "
                 "(which moves in lockstep with volume order) retains identifiable variance.")
    lines.append("")
    ...
    lines.append("- **Volume FE omitted by design** to recover experience-term identifiability. "
                 "This means volume-specific difficulty (some volumes are inherently harder to agree on) "
                 "is now in the residual rather than controlled.")
...
def main():
    ...
    # Save artifacts
    coef_df.to_csv(_OUT_DIR / "advisor_coefficient_table.csv", index=False)
    var_df.to_csv(_OUT_DIR / "advisor_variance_decomposition.csv", index=False)
    vif_df.to_csv(_OUT_DIR / "advisor_vif_table.csv", index=False)

    report = render_report(df, model, coef_df, var_df, vif_df, multi, resid)
    (_OUT_DIR / "advisor_report.md").write_text(report)

    print(f"Wrote advisor report to {_OUT_DIR / 'advisor_report.md'}")
    print()
